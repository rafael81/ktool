...
The current improvement loop connects 사진 용량 줄이기 to JPG PDF 변환 more tightly because compressed multi-image submissions need a visible handoff, not just a generic related-tool link.
...
- JPG PDF 변환: improved after the image-tool cluster with drag reorder, per-image removal, a preflight checklist, and a compressed-image arrival cue because page order, save-readiness, and post-compression handoff mistakes are direct submission-prep failure points.
