...
107. Retighten the `제출용 파일 준비` hub again after problem pages matured: shorten the first-screen copy, add first-screen problem cards for PDF bundling, 1MB compression, HEIC, format, pixel, crop, and rotation blockers, and track `prep_problem_click` with stable problem/tool metadata.
108. Add first-screen homepage problem starts for the mature problem pages: PDF bundling, 1MB compression, HEIC submission, and nameplate stamp blockers now appear under the search box with `home_problem_start_click` metadata, without adding long explanatory home sections.
